...
class Email{
    ...
    public function __construct($email, $hash, $type='verify'){
        $this->email = $email;
        $this->hash = $hash;
...
public function send(){
    ...
    */
        ...
        $mail->addAddress($this->email, 'Broli');                       // Add a recipient
...
public function send($email, $hash){
    ...
    // https://www.danielemanzi.it/  |||  http://localhost:3000/
    ...
    //http://localhost:3000/auth/verify/?email=dmanzi83@hotmail.it&hash=a597e50502f5ff68e3e25b9114205d4a
    ...
    //  GMAIL SMTP
        ...
        //Content
        $mail->isHTML(true);                                  // Set email format to HTML
        $mail->Subject = 'Prova 10';// Crea una nuova password // Benvenuto in danielemanzi.it // Account su danielemanzi.it cancellato
        $body = require_once 'app\views\view-auth\email.tpl.php'; //
        $mail->Body = $body;
        $mail->AltBody = strip_tags($body);
        $mail->send();
